...
export default function CryptoSalesList() {
  const t = useTranslations('sales')
  const { data: sales = [], isLoading } = useCryptoSalesQuery()
        ...
        <TableBody>
          {ordered.map((sale) => (
            <SalesRow
              key={sale.id}
              label={sale.symbol}
              quantity={sale.quantity.toLocaleString(undefined, { maximumFractionDigits: 8 })}
