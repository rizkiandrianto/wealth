    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <Card className="w-full max-w-lg p-6 space-y-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold">Sell {ticker}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="bg-muted/50 p-3 rounded-lg text-sm space-y-1">
            <p><span className="text-muted-foreground">Available:</span> <span className="font-semibold">{totalQty.toFixed(2)} {market === 'IDX' ? 'lot' : 'shares'}</span></p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium mb-2">Quantity ({market === 'IDX' ? 'lot' : 'shares'})</label>
              <Input
                type="number"
                step="0.01"
                value={quantity}
            ...
            <div>
              <label className="block text-sm font-medium mb-2">Sale Price (per share)</label>
              <Input
                type="number"
                step="0.01"
                value={salePrice}
                ...
                return (
                  <div key={c.lot.id} className="flex items-center justify-between gap-2 py-1 border-b last:border-b-0">
                    <div className="text-xs">
                      <p className="font-medium">Lot #{idx + 1} • {c.consumedQty} of {c.lot.quantity} {market === 'IDX' ? 'lot' : ''}</p>
