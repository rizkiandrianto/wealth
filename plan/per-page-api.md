...
| Page | Required APIs |
|---|---|
| `/` (Dashboard) | `/api/accounts`, `/api/transactions?limit=5`, `/api/stocks`, `/api/crypto`, `/api/gold`, `/api/stocks/sales?aggregate=sum`, `/api/crypto/sales?aggregate=sum`, `/api/gold/sales?aggregate=sum`, `/api/market/prices` |
